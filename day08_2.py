...
for line in input.split("\n"):
    var1, inc_or_dec, amount1, _, var2, signal, amount2 = line.split(" ")

    # check variables already in vars
    if var1 not in vars.keys():
        vars[var1] = 0
    if var2 not in vars.keys():
        vars[var2] = 0

    if (signal == ">" and vars[var2] > int(amount2)) or \
       (signal == "<" and vars[var2] < int(amount2)) or \
       (signal == ">=" and vars[var2] >= int(amount2)) or \
       (signal == "<=" and vars[var2] <= int(amount2)) or \
       (signal == "==" and vars[var2] == int(amount2)) or \
       (signal == "!=" and vars[var2] != int(amount2)):
        condition = True
    else:
        condition = False

    if condition and inc_or_dec == "inc":
        vars[var1] += int(amount1)
    elif condition and inc_or_dec == "dec":
        vars[var1] -= int(amount1)

    result = max(result, max(vars.iteritems(), key=operator.itemgetter(1))[1])
